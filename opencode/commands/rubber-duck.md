...
- **Mode A (Plan Critique)** — if `$ARGUMENTS` describes a proposed approach, design, or plan not yet implemented
- **Mode B (Code Critique)** — if `$ARGUMENTS` contains actual code, file paths, or references to written implementation
- **Mode C (Quack Protocol)** — if `$ARGUMENTS` contains "quack this", "explain to the duck", "rubber duck this", or similar phrasing
